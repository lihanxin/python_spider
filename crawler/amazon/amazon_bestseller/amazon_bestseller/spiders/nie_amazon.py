...
def table_data(response,trs_bestsell1):
...
class NieAmazonSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in range(len(trs_bestsell)):
            ...
            'product_name_one_url':product_name_one_url,"level":level,"higher_ups":higher_ups,"end":end1}))

    def child_father(self, response):
        soup = BeautifulSoup(response.text, "html.parser")
        try:
            childs = soup.find(name="span", attrs={"class": "zg_selected"}).parent.parent.find('ul').find('li').text
            if childs == '':
                print("节点空字符串")
            end_sign = '0' if childs is not None else '1'
        except:
            end_sign='1'
        return end_sign
    ...
    def parse_item_son_last(self, response):
        ...
        for i in range(len(trs_bestsell)):
            ...
            yield scrapy.Request(url=product_name_two_url, callback=self.parse_item_son_last2,
                                 meta=({'product_name_two': a2, 'product_name_two_url': a2_url, "level": level2,"higher_ups":response.meta['product_name_one']}))
    ...
    def parse_item_url2(self, response):
        ...
        for url in nie_url:
            yield scrapy.Request(url=url, callback=self.parse_item_son_last2,
                                 meta=({'product_name_two': a2, 'product_name_two_url': a2_url,"level":level,"higher_ups":higher_ups}))
    ...
    def parse_item_son_last2(self, response):
        """获取2级页面数据"""
        end2 = self.child_father(response)  # 标识最后一个节点
        trs_bestsell2 = response.xpath('//*[@id="zg-ordered-list"]/li')
        for i in range(len(trs_bestsell2)):
            item = AmazonBestsellerItem()
            price = ''.join(trs_bestsell2[i].xpath('.//span[@class="p13n-sc-price"]/text()').extract())
            if len(trs_bestsell2[i].xpath('.//span[@class="p13n-sc-price"]/text()').extract())>9:
                print("=====")
            image_url = trs_bestsell2[i].xpath('./span/div/span/a[1]/span/div/img/@src') .extract_first()
            reviews = trs_bestsell2[i].xpath('./span/div/span/div/a[2]/text() | ./span/div/span/div[1]/a[2]/text()').extract_first()
            title = trs_bestsell2[i].xpath("./span/div/span/a/div/text()").extract_first().strip()
            product_url = "https://www.amazon.com"+ trs_bestsell2[i].xpath("./span/div/span/a[1]/@href").extract_first()
            product_location =  trs_bestsell2[i].xpath("./span/div/div/span[1]/span/text()").extract_first()
            scrore = trs_bestsell2[i].xpath("./span/div/span/div[1]/a[1]/i/span/text() | ./span/div/span/div[2]/a[1]/i/span/text()").extract_first()
            crawling_time = str(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
        ...
        two_page_url = response.xpath("//*[@id='zg-center-div']/div[2]/div/ul/li[4]/a/@href").extract_first()  # 第二页网址
        if two_page_url:
            b2 = response.meta['product_name_two'];b2_level = response.meta['level'];higher_ups2 = response.meta['higher_ups']
            meta2=copy.copy({'product_name_two': b2, 'product_name_two_url': two_page_url, "level": b2_level,"higher_ups": higher_ups2})
            yield scrapy.Request(url=two_page_url, callback=self.parse_item_son_last2,meta=meta2)


        """获取3级页面url"""
        trs_bestsell = response.xpath('//*[@id="zg_browseRoot"]/ul/ul/ul/li')
        higher_ups = response.meta['product_name_two']  # 2级名称
        for i in range(len(trs_bestsell)):
            item = AmazonBestsellerItem()
            ...
            yield scrapy.Request(url=product_name_three_url, callback=self.parse_item_son_last3,
                                 meta=({'product_name_three': a2, 'product_name_three_url': a2_url, "level": level3,
    ...
    def parse_item_son_last3(self, response):
        """获取3级页面数据"""
        end3 = self.child_father(response)  # 标识最后一个节点
        ...
        two_page_url = response.xpath("//*[@id='zg-center-div']/div[2]/div/ul/li[4]/a/@href").extract_first()  # 第二页网址
        if two_page_url:
            b3=response.meta['product_name_three'];b3_level=response.meta['level'];higher_ups3=response.meta['higher_ups']
            meta3 = copy.copy({'product_name_three': b3, 'product_name_three_url': two_page_url,"level": b3_level,"higher_ups": higher_ups3})
            yield scrapy.Request(url=two_page_url, callback=self.parse_item_son_last3,meta=meta3)

        """获取4级页面url"""
        trs_bestsell = response.xpath('//*[@id="zg_browseRoot"]/ul/ul/ul/ul/li')
        higher_ups = response.meta['product_name_three']  # 3级名称
        for i in range(len(trs_bestsell)):
            item = AmazonBestsellerItem()
            ...
            a2 = product_name_four
            a2_url = product_name_four_url
            level4 = "4"
            print("4级", a2, a2_url)
            yield scrapy.Request(url=product_name_four_url, callback=self.parse_item_son_last4,
                                 meta=({'product_name_four': a2, 'product_name_four_url': a2_url, "level4": level4,
                                        "higher_ups4": higher_ups}))
    ...
    def parse_item_son_last4(self, response):
        """获取4级页面数据"""
        end4 = self.child_father(response)  # 标识最后一个节点
        ...
        two_page_url = response.xpath("//*[@id='zg-center-div']/div[2]/div/ul/li[4]/a/@href").extract_first()  # 第二页网址
        if two_page_url:
            b4 = response.meta['product_name_four'];b4_level = response.meta['level4'];higher_ups4 = response.meta['higher_ups4']
            meta4 = copy.copy({'product_name_four': b4, 'product_name_four_url': two_page_url, "level4": b4_level,"higher_ups4": higher_ups4})
            yield scrapy.Request(url=two_page_url, callback=self.parse_item_son_last4, meta=meta4)
    ...
    def parse_item_son_last5(self, response):
        """获取5级页面数据"""
        end5 = self.child_father(response)  # 标识最后一个节点
        ...
        two_page_url = response.xpath("//*[@id='zg-center-div']/div[2]/div/ul/li[4]/a/@href").extract_first()  # 第二页网址
        if two_page_url:
            b5 = response.meta['product_name_five'];b5_level = response.meta['level5'];higher_ups5 = response.meta['higher_ups5']
            meta5 = copy.copy({'product_name_five': b5, 'product_name_five_url': two_page_url, "level5": b5_level,"higher_ups5": higher_ups5})
            yield scrapy.Request(url=two_page_url, callback=self.parse_item_son_last5, meta=meta5)
    ...
    def parse_item_son_last6(self, response):
        """获取6级页面数据"""
        end6 = self.child_father(response) # 标识最后一个节点
        ...
        two_page_url = response.xpath("//*[@id='zg-center-div']/div[2]/div/ul/li[4]/a/@href").extract_first()  # 第二页网址
        if two_page_url:
            b6 = response.meta['product_name_six'];b6_level = response.meta['level6'];higher_ups6 = response.meta['higher_ups6']
            meta6 = copy.copy({'product_name_six': b6, 'product_name_six_url': two_page_url, "level6": b6_level,
                               "higher_ups6": higher_ups6})
            yield scrapy.Request(url=two_page_url, callback=self.parse_item_son_last6, meta=meta6)
    ...
    def parse_item_son_last7(self, response):
        """获取7级页面数据"""
        end7 = self.child_father(response)  # 标识最后一个节点
        ...
        two_page_url = response.xpath("//*[@id='zg-center-div']/div[2]/div/ul/li[4]/a/@href").extract_first()  # 第二页网址
        if two_page_url:
            b7 = response.meta['product_name_seven'];b7_level = response.meta['level7'];higher_ups7 = response.meta['higher_ups7']
            meta7 = copy.copy({'product_name_seven': b7, 'product_name_seven_url': two_page_url, "level7": b7_level,"higher_ups7": higher_ups7})
            yield scrapy.Request(url=two_page_url, callback=self.parse_item_son_last7, meta=meta7)
